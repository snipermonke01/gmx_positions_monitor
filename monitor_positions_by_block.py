...
def fetch_latest_block(web3_obj, first_block):
    return web3_obj.eth.block_number if first_block is None else web3_obj.eth.block_number


def fetch_contract_events(contract_obj, first_block, latest_block):
    logging.info(f"Making query between blocks: {first_block} to {latest_block}")
    return contract_obj.events.EventLog1.get_logs(from_block=first_block, to_block=latest_block)
...
def main(chain):
    ...
    contract_obj = get_event_emitter_contract(config)
    tokens_api_dict = get_tokens_address_dict(config.chain)
    first_block = None

    while True:
        try:
            latest_block = fetch_latest_block(web3_obj, first_block)
            events = fetch_contract_events(contract_obj, first_block, latest_block)
